...
#
# 1) Sanitize /etc/kubernetes/kubelet-env by stripping mistaken node-role…/worker-dpu.
...
#    - Ensure CUSTOM_KUBELET_LABELS includes:
#      feature.node.kubernetes.io/dpu-enabled=true,k8s.ovn.org/dpu-host=
#
# Non-DPU workers: do not touch kubelet-env for pool labels (only sanitize disallowed role key if file exists).
#
...
LEGACY_NON_KUBELET_DPU_ENTRY="k8s.ovn.org/dpu-host="
...
sanitize_forbidden_role_from_kubelet_env() {
  ...
  if ! grep -qF "${FORBIDDEN_ROLE_KEY}=" "$KUBELET_ENV"; then
    return 0
  fi
...
ensure_dpu_host_in_kubelet_env() {
  local val new_val="" p k found_dpu=0 found_legacy=0 tmp
  if [[ ! -f "$KUBELET_ENV" ]]; then
    echo "CUSTOM_KUBELET_LABELS=${DPU_WORKER_POOL_LABEL_ENTRY},${LEGACY_NON_KUBELET_DPU_ENTRY}" >"$KUBELET_ENV"
  ...
  for p in "${parts[@]}"; do
    ...
    if [[ "$k" == "$LEGACY_NON_KUBELET_DPU_KEY" ]]; then
      found_legacy=1
    fi
  done
  if [[ "$found_dpu" -eq 1 && "$found_legacy" -eq 1 ]]; then
    return 0
  fi
  new_val="$val"
  if [[ "$found_dpu" -eq 0 ]]; then
    new_val="${new_val},${DPU_WORKER_POOL_LABEL_ENTRY}"
  fi
  if [[ "$found_legacy" -eq 0 ]]; then
    new_val="${new_val},${LEGACY_NON_KUBELET_DPU_ENTRY}"
  fi
  new_val="${new_val#,}"
  tmp=$(mktemp)
  grep -v '^CUSTOM_KUBELET_LABELS=' "$KUBELET_ENV" >"$tmp" || true
